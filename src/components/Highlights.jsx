        ...
        <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-12 lg:gap-32'>
          {highlightDishes.map((dish) => (
            <div className='w-[300px] bg-[#495d57] text-white flex flex-col justify-between rounded-3xl' key={dish.id}>
              ...
              <div className='flex justify-around items-center text-2xl pt-2'>
                <h1 className='font-semibold'>{dish.name}</h1>
                <p className='font-semibold'>${dish.price}</p>
              </div>
              <p className='p-4 font-semibold'>{dish.description}</p>
              <button className='p-2 flex justify-center items-center bg-yellow-400 text-black font-semibold rounded-xl w-1/2 mb-4 mx-auto'>Order Now</button>
            </div>
          ))}
        </div>
      </section>
